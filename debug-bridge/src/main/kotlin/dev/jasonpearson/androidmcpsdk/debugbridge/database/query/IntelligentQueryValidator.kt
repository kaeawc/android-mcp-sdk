...
class IntelligentQueryValidator(private val schemaCache: DatabaseSchemaCache) {
    ...
    suspend fun validateQuery(
        ...
        withContext(Dispatchers.IO) {
            ...
            val schema =
                schemaCache.getOrLoadSchema(databaseUri)
                    ?: return@withContext QueryValidationResult(
                        ...
                        estimatedCost =
                            QueryCost(0, ComplexityLevel.LOW, IndexUsage.UNKNOWN, ScanType.UNKNOWN),
    ...
    suspend fun validateQueryForPagination(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val indexRecommendations =
                    suggestPaginationIndexes(parsedQuery, sortColumns, schema)
    ...
    private fun parseQuery(query: String): ParsedQuery {
        ...
        return ParsedQuery(
            ...
            hasSubqueries =
                query.uppercase().contains("SELECT") &&
                    query.count { it.toString().uppercase() == "SELECT" } > 1,
            sql = query,
        )
    }
    ...
    private fun validateQueryAgainstSchema(
        ...
        parsedQuery.columns.forEach { columnName ->
            val columnExists =
                parsedQuery.tables.any { tableName ->
        ...
        parsedQuery.whereColumns.forEach { columnName ->
            val needsIndex =
                parsedQuery.tables.any { tableName ->
                    ...
                    val hasIndex =
                        schema.indexes.values.any { index ->
            ...
            if (needsIndex) {
                warnings.add(
                    QueryWarning(
                        ...
                        suggestion =
                            "Consider adding an index on '$columnName' for better performance",
                    )
                )
            }
        }
    ...
    private fun estimateQueryCost(
        ...
        val estimatedRows =
            parsedQuery.tables.sumOf { tableName ->
                schema.tables[tableName]?.estimatedRowCount ?: 1000L
            }

        val complexity =
            when {
                parsedQuery.hasSubqueries || parsedQuery.joinColumns.isNotEmpty() ->
                    ComplexityLevel.HIGH
                parsedQuery.whereColumns.size > 2 || parsedQuery.groupByColumns.isNotEmpty() ->
        ...
        return QueryCost(
            ...
            scanType =
                if (indexUsage != IndexUsage.NONE) ScanType.INDEX_SCAN else ScanType.TABLE_SCAN,
        )
    }
    ...
    private fun recommendIndexes(
        ...
        parsedQuery.tables.forEach { tableName ->
            val whereColumnsForTable =
                parsedQuery.whereColumns.filter { columnName ->

...
class NetworkReplayEngineTest {
    ...
    fun `replayRequest should create replay result`() = runTest {
        // Given
        val originalRequest = createTestNetworkRequest(
            id = "test_1",
    ...
    fun `replayRequest should apply modifications`() = runTest {
        ...
        val modifications = NetworkReplayEngine.RequestModifications(
    ...
    fun `batchReplay should handle multiple requests`() = runTest {
        ...
        val config = NetworkReplayEngine.BatchConfig(
            concurrency = 1,
    ...
    fun `loadTest should generate statistics`() = runTest {
        // Given
        val request = createTestNetworkRequest(
            id = "load_test",
            url = "https://httpbin.org/get",
            method = "GET"
        )
        
        val config = NetworkReplayEngine.LoadTestConfig(

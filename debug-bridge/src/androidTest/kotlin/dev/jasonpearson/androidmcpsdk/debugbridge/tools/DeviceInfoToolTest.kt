...
class DeviceInfoToolTest : ToolValidationFramework() {
    ...
    fun testDeviceInfoDataAccuracy() {
        ...
        assertTrue("Result should contain model", resultText.contains("Model: ${Build.MODEL}"))
        assertTrue("Result should contain brand", resultText.contains("Brand: ${Build.BRAND}"))
        assertTrue("Result should contain device", resultText.contains("Device: ${Build.DEVICE}"))
        assertTrue(
    ...
    fun testDeviceInfoConsistency() {
        ...
        assertTrue("All results should be identical", results.all { it.result == firstResult })
    }

    @Test
    fun testDeviceInfoWithInvalidArguments() {
        // Test with invalid arguments (should ignore them gracefully)
        val invalidArgs = mapOf("invalid_param" to "invalid_value", "another_param" to 12345)

...
class ResourceProviderTest {
    ...
    fun setUp() {
        ...
        if (context !is ResourceProviderContainer) {
            throw IllegalStateException("Test Application class must implement ResourceProviderContainer")
        }
        ...
        testPublicDownloadsDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).also { it.mkdirs() }
    ...
    fun `getAndVerifyAccessibleFile - allows app-internal file`() {
        val testFile = File(testAppFilesDir, "internal_test.txt").apply { writeText("internal data") }
    ...
    fun `getAndVerifyAccessibleFile - allows app-external file`() {
        val testFile = File(testAppExternalFilesDir, "external_test.txt").apply { writeText("external data") }
        val fileUri = "file://${testFile.absolutePath}"
        val accessibleFile = resourceProvider.getAndVerifyAccessibleFile(fileUri)
    ...
    fun `getAndVerifyAccessibleFile - allows file in public downloads (Robolectric context)`() {
        val testFile = File(testPublicDownloadsDir, "public_test.txt").apply { writeText("public data") }
        val fileUri = "file://${testFile.absolutePath}"
        val accessibleFile = resourceProvider.getAndVerifyAccessibleFile(fileUri)
    ...
    fun `getAndVerifyAccessibleFile - denies access to arbitrary path outside sandbox`() {
        ...
        val arbitraryDir = File(tempDir, "arbitrary_test_dir_${System.currentTimeMillis()}").apply { mkdirs() }
    ...
    fun `getAndVerifyAccessibleFile - denies access for non-file URI`() {
        ...
        assertNull("Non-file URI should be denied", resourceProvider.getAndVerifyAccessibleFile(httpUri))
    }

    @Test
    fun `getAndVerifyAccessibleFile - returns null for invalid URI path format`() {
        val invalidUri = "file:///invalid path with spaces.txt"
        assertNull("Invalid URI path format should return null", resourceProvider.getAndVerifyAccessibleFile(invalidUri))
    }

    // --- ResourceSubscriptionManager Tests (via ResourceProvider) ---
    @Test
    fun `subscribeToResource - file URI starts FileObserver`() = testScope.runTest {
    ...
    @Test
    fun `subscribeToResource - non-existent file URI observes parent directory`() = testScope.runTest {
    ...
    @Test
    fun `subscribeToResource - dynamic URI starts polling`() = testScope.runTest {
    ...
    @Test
    fun `unsubscribeFromResource - stops FileObserver and polling`() = testScope.runTest {
        ...
        assertTrue(pollingJobBefore?.isCancelled == true || pollingJobBefore?.isCompleted == true)
        testFile.delete()
    }

    @Test
    fun `resourceUpdates flow emits URI on change - file observer`() = testScope.runTest {
        val testFile = File(testAppFilesDir, "notify_test.txt").apply { writeText("content1") }
        val fileUri = "file://${testFile.absolutePath}"
        var receivedUri: String? = null
        val collectJob = launch { resourceProvider.resourceUpdates.first { it == fileUri }.also { receivedUri = it } }
        resourceProvider.subscribe(fileUri)
        ...
        advanceTimeBy(ResourceSubscriptionManager.DEBOUNCE_TIME_MS + 150) // Debounce time + buffer
        runCurrent()

        // Note: In a real test environment, this would work, but Robolectric's FileObserver simulation is limited
    ...
    @Test
    fun `MAX_FILE_OBSERVERS limit falls back to dynamic polling`() = testScope.runTest {
        ...
        val uris = (1..55).map { i ->
            File(testAppFilesDir, "limit_test_$i.txt").apply { writeText("data") }
        ...
        val fileSubscriptions =
            subscriptions.filter { it.type == ResourceSubscriptionManager.SubscriptionType.FILE }
        val dynamicSubscriptions =
            subscriptions.filter { it.type == ResourceSubscriptionManager.SubscriptionType.DYNAMIC }
    ...
    @Test
    fun `dynamic polling uses exponential backoff on error`() = testScope.runTest {
        ...
        ResourceProvider::class.java.getDeclaredField("subscriptionManager").apply { isAccessible = true }.set(tempResourceProvider, spiedManager)

        // Mock the method to always throw an exception
        coEvery { spiedManager invoke "readAndProcessDynamicResource" withArguments listOf(dynamicUri) } throws IOException("Poll fail")
        
        tempResourceProvider.subscribe(dynamicUri)
        val sub = spiedManager.getSubscriptionForTest(dynamicUri)
        assertNotNull(sub); sub!!
        ...
        coVerify(atLeast = 1) { spiedManager invoke "readAndProcessDynamicResource" withArguments listOf(dynamicUri) }
    ...
    private fun setCompanionObjectField(companionClass: Class<*>, fieldName: String, newValue: Any) {
        throw UnsupportedOperationException("Reflection-based field access removed for stability")
    }
}
...
internal fun ResourceSubscriptionManager.getSubscriptionForTest(uri: String): ResourceSubscriptionManager.ActiveSubscription? {
    return ResourceSubscriptionManager::class.java.getDeclaredField("subscriptions").run {
        isAccessible = true
        @Suppress("UNCHECKED_CAST")
        (get(this@getSubscriptionForTest) as? ConcurrentHashMap<String, ResourceSubscriptionManager.ActiveSubscription>)?.get(uri)
    }
}

...
class McpFeaturesIntegrationTest {
    ...
    fun `executeAndroidTool should execute system_time tool`() = runTest {
        ...
        val resultString = result.result as String
        assertTrue("Result should contain time information",
    ...
    fun `executeAndroidTool should handle unknown tool gracefully`() = runTest {
        ...
        assertTrue("Error should mention unknown tool",
    ...
    fun `addAndroidTool should allow adding custom tools`() = runTest {
        ...
        assertEquals("Custom tool should return expected result", "Test output: hello", result.result)
    }

    @Test
    fun `addAndroidTool should allow multiple custom tools`() {
        val tool1 = AndroidTool(
            name = "tool1",
            description = "First test tool",
            parameters = emptyMap()
        ) { _, _ -> "Tool 1 result" }

        val tool2 = AndroidTool(
            name = "tool2", 
        ...
        val tools = manager.getAndroidTools()
        assertTrue("Both tools should be added", 
    ...
    fun `startServer should start server with coroutines`() = runTest {
        ...
        try {
            ...
            assertTrue("Exception should be related to test environment constraints", 
    ...
    fun `broadcastMessage should handle message broadcasting`() = runTest {
        ...
        try {
            ...
            if (result.isFailure) {
                ...
                assertTrue("Should fail gracefully when transports not running",
                           exception is IllegalStateException)
            }
        } catch (e: Exception) {
            // Broadcasting exceptions are acceptable in test environment
            assertTrue("Exception should be related to transport state", 
                       e.message?.contains("running") == true ||
    ...
    fun `hasSDKIntegration should report SDK availability`() {
        ...
        assertTrue("hasSDKIntegration should return a boolean value", 

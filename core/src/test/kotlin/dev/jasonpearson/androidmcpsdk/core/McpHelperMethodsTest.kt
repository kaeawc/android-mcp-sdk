...
import dev.jasonpearson.androidmcpsdk.core.lifecycle.McpLifecycleManager
...
class McpHelperMethodsTest {
    ...
    fun `test addMcpTool adds custom tool successfully`() {
        ...
        val handler: suspend (Map<String, Any>) -> CallToolResult = { _ ->
            CallToolResult(content = listOf(TextContent(text = "Test result")), isError = false)
        }
    ...
    fun `test removeMcpTool removes custom tool successfully`() {
        ...
        assertTrue(
            ...
            manager.getMcpTools().any { it.name == "removable_tool" },
        )
    ...
    fun `test addMcpResource adds custom resource successfully`() {
        ...
        assertTrue(
            "Resource should be added to the list",
            resources.any { it.uri == "test://resource/1" },
        )
    }
    ...
    fun `test getMcpPrompt retrieves prompt with arguments`() = runBlocking {
        ...
        val result =
            manager.getMcpPrompt("parameterized_prompt", mapOf("message" to "Hello, World!"))

        assertEquals(
            "Description should match",
            "Parameterized prompt response",
            result.description,
        )
        assertTrue(
    ...
    fun `test addSimpleTool creates and adds tool successfully`() {
        ...
        assertTrue(
            "Simple tool should be added as Android tool",
    ...
    fun `test addSimplePrompt creates and adds prompt successfully`() {
        ...
        val prompts = manager.getMcpPrompts()
        assertTrue(
            "Simple prompt should be added to the list",

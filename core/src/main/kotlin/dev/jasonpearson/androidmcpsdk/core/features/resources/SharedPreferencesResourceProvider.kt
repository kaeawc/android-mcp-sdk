        ...
        withContext(Dispatchers.IO) {
            return@withContext when {
                ...
                uri.startsWith("android://preferences/") -> readSpecificPreferencesFile(uri)
                    ...
                    AndroidResourceContent(uri = uri, text = "Unknown preferences resource: $uri")
            }
        }
    ...
    private suspend fun readAllPreferencesFiles(): AndroidResourceContent =
        withContext(Dispatchers.IO) {
            try {
                ...
                val jsonContent =
                    Json.encodeToString(
                        mapOf("preferencesFiles" to prefsFiles, "totalFiles" to prefsFiles.size)
                    )
    ...
    private suspend fun readSpecificPreferencesFile(uri: String): AndroidResourceContent =
        withContext(Dispatchers.IO) {
            try {
                ...
                AndroidResourceContent(uri = uri, text = jsonContent, mimeType = "application/json")
            } catch (e: Exception) {

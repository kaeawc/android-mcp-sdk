 ...
 *
 * This transport creates a WebSocket server that MCP clients can connect to. It supports multiple
 * concurrent client connections and bidirectional communication.
 */
class WebSocketTransport(private val config: TransportConfig = TransportConfig()) : McpTransport {
        ...
        if (_isRunning.compareAndSet(false, true)) {
            ...
            server =
                embeddedServer(Netty, port = config.port, host = config.host) {
                    ...
                    routing { webSocket(WEBSOCKET_PATH) { handleWebSocketConnection(this) } }
                }
    ...
    private suspend fun handleWebSocketConnection(session: WebSocketSession) {
        ...
        try {
            for (frame in session.incoming) {
                when (frame) {
                    ...
                    is Frame.Close -> {
                        Log.i(
                            TAG,
                            "WebSocket connection $connectionId closed: ${frame.readReason()}",
                        )
                        break
                    }
                    else -> {
            ...
            Log.i(
                TAG,
                "WebSocket connection $connectionId disconnected (remaining: ${connectionCount.get()})",
            )
        }
    }

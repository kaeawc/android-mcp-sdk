 ...
 *
 * This manager can run multiple transports simultaneously (e.g., both WebSocket and HTTP/SSE) and
 * provides a unified interface for MCP communication.
 */
class TransportManager {
    ...
    suspend fun startAll(): Result<Unit> = runCatching {
        if (_isRunning.compareAndSet(false, true)) {
            ...
            transports.forEach { (name, transport) ->
                try {
                    ...
                    if (result.isSuccess) {
                        ...
                        Log.e(
                            TAG,
                            "Transport '$name' failed to start: ${result.exceptionOrNull()?.message}",
                        )
                    }
                } catch (e: Exception) {
            ...
            Log.i(
                TAG,
                "Transport manager started with ${transports.size - failures.size}/${transports.size} transports",
            )
        } else {
            ...
            transports.forEach { (name, transport) ->
                try {
                    ...
                    if (result.isSuccess) {
                        ...
                        Log.e(
                            TAG,
                            "Transport '$name' failed to stop: ${result.exceptionOrNull()?.message}",
                        )
                    }
                } catch (e: Exception) {
    ...
    suspend fun broadcast(message: String): Result<Unit> = runCatching {
            ...
            if (transport.isRunning) {
                try {
                    ...
                    if (result.isSuccess) {
                        ...
                        Log.w(
                            TAG,
                            "Failed to send message via transport '$name': ${result.exceptionOrNull()?.message}",
                        )
                    }
                } catch (e: Exception) {
        ...
        Log.d(
            TAG,
            "Message broadcast to ${sendResults.size - failures.size}/${sendResults.size} transports",
        )
    }
    ...
    fun getConnectionInfo(): Map<String, Any> {
        return mapOf(
            ...
            "transports" to transports.mapValues { (_, transport) -> transport.getConnectionInfo() },
        )
    }
    ...
    fun setupDefaultTransports(
        ...
        Log.i(
            TAG,
            "Default transports configured: WebSocket on port $webSocketPort, HTTP/SSE on port $httpPort",
        )
    }
}
